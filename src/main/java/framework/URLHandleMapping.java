...
public class URLHandleMapping {
	...
	public URLHandleMapping(Map<String, String> map) throws Exception {
		mappings = new HashMap<>();
		ctrlNameList = new ArrayList<>();
		
		// scanPackage : kr.co.mlec
		getCtrlNameList(map.get("scanPackage"));
		
		addMap(map.get("configLocation"));
	}

	private void addMap(String configLocation) throws Exception {
		ApplicationContext context = new ClassPathXmlApplicationContext(configLocation);
		
		for (String ctrlName : ctrlNameList) {
			Class<?> clz = Class.forName(ctrlName.trim());
			Object target = context.getBean(ctrlName.trim());
